...
from streamlit_option_menu import option_menu  # Importing additional functionality for option menus in Streamlit

from PIL import Image  # Importing the Python Imaging Library for image processing


import datetime  # Importing datetime module for handling date and time data
...
warnings.filterwarnings("ignore")  # Ignore warning messages to maintain clean output
...
# Importing regression metrics from scikit-learn
from sklearn.metrics import mean_squared_error, mean_absolute_error, r2_score

# Importing classifier models from scikit-learn
from sklearn.svm import SVC
from sklearn.ensemble import RandomForestClassifier

# Importing classification metric from scikit-learn
...
with st.sidebar:
    #image insertion
    image_path = (r"C:\Users\Acer\Desktop\Python-V\Industrial_Copper_Modelling\image.jpg")  
    image = Image.open(image_path)

    # Display the image using st.image()
    st.sidebar.image(image)

    selected = option_menu("Get Started Here!", ['HOME',"PRICE PREDICTION","STATUS PREDICTION"],
                        icons=["house",'cash-coin','trophy',"check-circle"],
...
if selected=='HOME':
        ...
        st.write('- Random forest is a commonly-used machine learning algorithm which combines the output of multiple decision trees to reach a single result. Its ease of use and flexibility have fueled its adoption.')
...
if selected=='STATUS PREDICTION':
    try:
        if 'STATUS PREDICTION':
            ...
            st.write('#### ***<span style="color:BLACK">Fill all the fields and Press the below button to view the status :violet[WON] / :red[LOST] of copper in the desired time range</span>***',
                        unsafe_allow_html=True)
            st.write('')
            st.write('')
